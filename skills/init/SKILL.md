...
运行 `speccode resolve-speccode-dir --cwd .` 获取 `speccodeDir`。
运行 `speccode read-config --cwd .` 判断是否已初始化:
- `config` 为 null → 全新 init(走"全新流程")
- `config` 非 null → 二次 init(走"幂等流程")

## 全新流程

1. **探测远端与 pr_tool**:运行 `speccode detect-remote --cwd .`,得到 `prToolGuess` 与 `installed`。
   - 若 `installed=false` 且 `prToolGuess≠none`:告知用户"探测到应使用 <tool>,但未检测到该 CLI",询问是否降级为 `none`。
   - 向用户提问确认(给出选项)最终 `pr_tool`(gh / glab / none)。
2. **探测主干分支**:运行 `git symbolic-ref refs/remotes/origin/HEAD`(失败则回退询问);默认填 `trunk`,请用户确认。
3. **询问 worktree_dir**:worktree 存放的基础目录,默认 `.speccode/worktrees`,请用户确认或自定义(相对项目根的路径)。
4. **探测宿主**:运行 `speccode detect-host --cwd .` 得到 best-effort 启发结果(可 `--host <id>` 显式指定重新运行)。
   - 向用户提问确认(给出选项)宿主身份,枚举:`claude-code / codex / zcode / opencode / pi / kimi-code / generic`;探测结果仅为预填,用户选择为准。
   - 探测结果 MUST NOT 未经确认写入;用户选 `generic` 或跳过 → 不写 `host` 字段(缺失即 generic 语义)。
5. **探测代码智能工具**:运行 `speccode detect-code-intel-tools --cwd .`(引擎按已确认的 `config.host` 分流;全新流程中此时 host 尚未落盘,按全量探测,工具的 ~/.claude 信号在 Claude Code 会话内本就有效)。
...
3. 用 `diffFields` 逐字段比较旧/新:
   - 值未变 → 跳过。
   - 值变化 → 向用户提问展示(给出选项) `[旧值] → [新值]`,询问"保持 / 改用新值 / 清除"。
   ...
   - 宿主切换注意:工具探测受 `config.host` 门控,二次 init 重跑探测时旧 host 仍在盘上——若用户要切换宿主(如 codex → claude-code),工具登记 diff 基于旧 host 的探测结果,建议写回新 host 后重跑本流程复核工具清单。
...
- 探测类结果(代码智能工具、pr_tool 猜测、宿主身份)一律经用户确认后才落盘——不确定就先询问,不猜测。
